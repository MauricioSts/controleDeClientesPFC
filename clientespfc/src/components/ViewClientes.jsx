...
function ViewClientes() {
  ...
  const toggleConcluido = async (cliente) => {
    try {
      ...
      toast.success(
        cliente.concluido ? "Cliente pendente" : "Cliente concluído ✅"
      );
    } catch (error) {
          ...
          <AnimatePresence>
            {clientes.map((cliente) => (
              <motion.li
                ...
                className={`flex flex-col md:flex-row justify-between items-start md:items-center bg-[#1A0841]/90 p-3 sm:p-4 rounded-lg border border-[#FF2E63]/20 hover:border-[#FF2E63]/50 transition-opacity ${
